# 基于最大熵原理的数学推理系统 - 项目工作流程文档

> **项目名称**: Entropy-Guided Reasoning (EGR)  
> **核心创新**: 将最大熵原理系统化应用于自动数学推理  
> **目标数据集**: Conic10K  
> **目标准确率**: 60-80% (Phase 1)

---

## 📊 系统架构总览

```
┌─────────────────────────────────────────────────────────────┐
│                    输入：数学问题                              │
│   fact_expressions + query_expressions + answer              │
└────────────────────┬────────────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────────────┐
│  模块1: 状态抽象器 (State Abstractor)                         │
│  ├─ 输入: fact_expressions, query_expressions               │
│  └─ 输出: 抽象状态向量                                         │
└────────────────────┬────────────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────────────┐
│  模块2: 定理选择器 (Theorem Selector)                         │
│  ├─ 输入: 当前状态                                            │
│  ├─ 模型: P(Y|X) - 最大熵分类器                               │
│  └─ 输出: 定理概率分布 + 当前状态熵                            │
└────────────────────┬────────────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────────────┐
│  模块3: 定理应用器 (Theorem Applicator)                       │
│  ├─ 输入: 当前状态 + 选定定理                                 │
│  └─ 输出: 新状态                                              │
└────────────────────┬────────────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────────────┐
│  模块4: 推理引擎 (Reasoning Engine)                           │
│  ├─ 策略: 信息增益最大化                                       │
│  ├─ 循环: 模块2 → 模块3 → 模块2...                            │
│  └─ 输出: 最终答案 + 推理路径                                  │
└────────────────────┬────────────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────────────┐
│                输出：预测答案 + 推理步骤                        │
└─────────────────────────────────────────────────────────────┘
```

---

## 🔧 主要工作模块详解

---

### **模块 0: 定理库构建** 

**工作目标**: 构建Conic10K领域的形式化定理库

#### 📝 工作流程

1. **数据分析**
   - 统计`train.json`中所有`process`字段
   - 提取常用推理步骤和定理模式
   - 识别定理使用频率

2. **定理形式化**
   - 定义定理前置条件（precondition）
   - 定义定理输出（output）
   - 设计定理优先级（3层：基础/中级/高级）

3. **定理实现**
   - 编写定理的符号执行逻辑
   - 实现前置条件检查函数
   - 编写单元测试

#### 📥 输入
- **数据**: `Conic10K/conic10k/train.json` 
- **人工知识**: 圆锥曲线数学定理

#### 📤 输出
```python
{
    "theorem_id": "T1_ellipse_abc",
    "name": "椭圆参数关系",
    "formula": "a² = b² + c²",
    "precondition": {
        "curve_type": "Ellipse",
        "required_params": ["any_two_of", ["a", "b", "c"]]
    },
    "output": {
        "type": "parameter_value",
        "produces": "missing_parameter"
    },
    "priority": "basic",
    "implementation": "function_reference"
}
```

#### ✅ 验收标准
- [ ] 至少15个基础定理
- [ ] 覆盖训练集中60%以上的推理步骤
- [ ] 每个定理有完整的前置条件定义
- [ ] 每个定理通过10个单元测试

#### ⏱️ 预计时间
**1周**

---

### **模块 1: 状态抽象器 (State Abstractor)**

**工作目标**: 将原始的`fact_expressions`抽象为有限维度的状态表示

#### 📝 工作流程

1. **特征提取**
   - 解析`fact_expressions`字符串
   - 识别曲线类型（Ellipse/Hyperbola/Parabola/Circle）
   - 提取已知信息（方程/参数/焦点/渐近线等）
   - 分类查询类型（离心率/方程/坐标/距离等）

2. **状态编码**
   - 将提取的特征转换为向量表示
   - 计算信息完整度分数（0-1）
   - 生成状态哈希（用于去重和索引）

3. **验证与统计**
   - 统计训练集的状态分布
   - 计算状态空间覆盖率

#### 📥 输入
```python
{
    "fact_expressions": "G: Hyperbola; Expression(G) = (x^2/16 - y^2/25 = 1); LeftFocus(G) = F1; ...",
    "query_expressions": "Eccentricity(G)"
}
```

#### 📤 输出
```python
{
    "state_vector": {
        "curve_type": "Hyperbola",
        "has_equation": True,
        "known_params": ["a=4", "b=5"],
        "has_focus": True,
        "query_type": "eccentricity",
        "completeness": 0.6
    },
    "state_hash": "HYP_EQ_AB_FOC_ECCENT",
    "state_embedding": [0.2, 0.8, 1.0, ...]  # 用于神经网络
}
```

#### ✅ 验收标准
- [ ] 覆盖率 > 95%（训练集样本能被成功抽象）
- [ ] 状态空间大小：500-1000个代表性状态
- [ ] 信息完整度估计与人工标注相关性 > 0.7

#### ⏱️ 预计时间
**1-1.5周**

#### 📂 代码文件
- `src/state_abstractor.py`
- `tests/test_state_abstractor.py`

---

### **模块 2: 训练数据构造器**

**工作目标**: 从Conic10K构造用于训练神经网络的(状态, 定理, 熵)三元组

#### 📝 工作流程

1. **推理步骤解析**
   - 使用LLM或规则方法解析`process`字段
   - 识别每一步使用的定理
   - 构建推理轨迹

2. **状态模拟**
   - 从初始状态开始
   - 逐步应用定理，生成中间状态
   - 记录每个状态-定理对

3. **熵标签生成**
   - 方法A: 剩余步数作为熵估计（简单）
   - 方法B: 基于信息完整度估计（更准确）

4. **数据增强**（可选）
   - 打乱推理顺序
   - 生成负样本（不适用的定理）

#### 📥 输入
```python
{
    "fact_expressions": "...",
    "query_expressions": "...",
    "process": "双曲线的渐近线方程为y=±(m/2)x，所以m=5.",
    "answer_expressions": "5"
}
```

#### 📤 输出
```python
[
    {
        "state": state_vector_0,
        "theorem_label": "T6_asymptote",
        "entropy_label": 2.0,  # 还需2步
        "is_applicable": True
    },
    {
        "state": state_vector_1,
        "theorem_label": "T15_substitution",
        "entropy_label": 1.0,  # 还需1步
        "is_applicable": True
    },
    ...
]
```

#### ✅ 验收标准
- [ ] 训练集大小：50,000+ 样本
- [ ] 验证集大小：10,000+ 样本
- [ ] 定理分布基本均衡（每个定理至少500个样本）
- [ ] 熵标签的合理性检查通过

#### ⏱️ 预计时间
**1.5-2周**

#### 📂 代码文件
- `src/data_constructor.py`
- `src/process_parser.py`

---

### **模块 3: 神经网络模型**

**工作目标**: 训练多任务学习模型，预测P(Y|X)和H(S)

#### 📝 工作流程

1. **模型设计**
   - Tokenizer：将状态向量转换为token序列
   - Encoder：Transformer编码器（6层）
   - Head 1：定理分类头（15-50类）
   - Head 2：熵回归头（1个数值）

2. **训练配置**
   - 损失函数：交叉熵（定理）+ MSE（熵）
   - 优化器：AdamW
   - 学习率调度：Warmup + Cosine Decay
   - 批大小：64
   - Epoch：50-100

3. **训练监控**
   - 定理选择准确率
   - 熵估计MAE/MSE
   - 验证集性能
   - 过拟合检测

4. **模型评估**
   - Top-1准确率（定理选择）
   - Top-3准确率
   - 熵估计的Pearson相关系数

#### 📥 输入
```python
{
    "train_data": [
        {"state_tokens": [...], "theorem_label": 3, "entropy_label": 2.1},
        ...
    ],
    "val_data": [...]
}
```

#### 📤 输出
```python
# 训练好的模型
model = EntropyGuidedReasoner(
    vocab_size=500,
    d_model=256,
    nhead=8,
    num_layers=6,
    num_theorems=15
)

# 推理时输出
{
    "theorem_probs": [0.05, 0.12, 0.58, ...],  # 15个定理的概率
    "entropy_estimate": 1.8  # 当前状态的熵估计
}
```

#### ✅ 验收标准
- [ ] 定理选择准确率（Top-1）> 70%
- [ ] 定理选择准确率（Top-3）> 85%
- [ ] 熵估计 MAE < 0.5
- [ ] 熵估计与实际步数的相关系数 > 0.75

#### ⏱️ 预计时间
**2-3周**

#### 📂 代码文件
- `src/models/egr_model.py`
- `src/models/tokenizer.py`
- `src/train.py`
- `configs/model_config.yaml`

---

### **模块 4: 定理应用器 (Theorem Applicator)**

**工作目标**: 符号执行引擎，给定状态和定理，计算新状态

#### 📝 工作流程

1. **前置条件检查**
   - 验证定理是否适用于当前状态
   - 检查所需参数是否已知
   - 检查曲线类型是否匹配

2. **符号推理**
   - 根据定理公式计算新信息
   - 可能涉及：
     - 数值计算（a²+b²=c²）
     - 符号替换（代入方程）
     - 逻辑推理（由定义得出性质）

3. **状态更新**
   - 将新推导的信息加入状态
   - 更新信息完整度
   - 重新计算状态向量

4. **验证**
   - 检查状态一致性
   - 检查是否产生矛盾

#### 📥 输入
```python
{
    "current_state": {
        "curve_type": "Hyperbola",
        "known_params": {"a": 4, "b": 5}
    },
    "theorem": "T2_hyperbola_abc"  # c² = a² + b²
}
```

#### 📤 输出
```python
{
    "new_state": {
        "curve_type": "Hyperbola",
        "known_params": {"a": 4, "b": 5, "c": 6.4}  # 新增c
    },
    "success": True,
    "derived_fact": "c = sqrt(41)"
}
```

#### ✅ 验收标准
- [ ] 支持15个基础定理的执行
- [ ] 每个定理通过10个测试用例
- [ ] 能够处理边界情况（如除以零、无解等）
- [ ] 执行速度 < 10ms/次

#### ⏱️ 预计时间
**1.5-2周**

#### 📂 代码文件
- `src/theorem_applicator.py`
- `src/symbolic_executor.py`
- `tests/test_theorem_applicator.py`

---

### **模块 5: 推理引擎 (Reasoning Engine)**

**工作目标**: 集成所有模块，实现完整的问题求解流程

#### 📝 工作流程

1. **初始化**
   - 抽象初始状态
   - 设置最大推理步数（如10步）

2. **迭代推理循环**
   ```
   for step in range(max_steps):
       # 2.1 模型预测
       theorem_probs, current_entropy = model(current_state)
       
       # 2.2 候选定理评估
       for each theorem:
           if 可应用:
               计算应用后的状态
               估计新状态的熵
               计算信息增益 = current_entropy - next_entropy
               
               综合得分 = λ₁·P(theorem|state) + λ₂·info_gain
       
       # 2.3 选择最优定理
       best_theorem = argmax(scores)
       
       # 2.4 应用定理
       current_state = apply_theorem(current_state, best_theorem)
       
       # 2.5 检查是否达到目标
       if is_solved(current_state):
           return extract_answer(current_state)
   ```

3. **答案提取**
   - 从最终状态中提取答案
   - 格式化为标准形式

4. **推理路径记录**
   - 记录每一步使用的定理
   - 记录熵的变化曲线

#### 📥 输入
```python
{
    "fact_expressions": "G: Ellipse; Expression(G) = (x^2/4 + y^2/3 = 1)",
    "query_expressions": "Eccentricity(G)",
    "answer_expressions": "sqrt(3)/3"  # 仅用于验证
}
```

#### 📤 输出
```python
{
    "predicted_answer": "sqrt(3)/3",
    "reasoning_path": [
        {"step": 1, "theorem": "T5_extract_params", "result": "a=2, b=sqrt(3)"},
        {"step": 2, "theorem": "T1_ellipse_abc", "result": "c=1"},
        {"step": 3, "theorem": "T4_eccentricity", "result": "e=sqrt(3)/3"}
    ],
    "entropy_trace": [2.5, 1.2, 0.3, 0.0],  # 熵的变化
    "is_correct": True,
    "num_steps": 3
}
```

#### ✅ 验收标准
- [ ] 在测试集上准确率 > 60%
- [ ] 平均推理步数 < 6步
- [ ] 95%的问题能在10步内完成或报告失败
- [ ] 推理路径可解释（每一步都有明确的定理）

#### ⏱️ 预计时间
**1-1.5周**

#### 📂 代码文件
- `src/reasoning_engine.py`
- `src/answer_extractor.py`
- `src/eval.py`

---

### **模块 6: 评估与分析**

**工作目标**: 在Conic10K测试集上评估系统性能，分析结果

#### 📝 工作流程

1. **性能评估**
   - 准确率计算
   - 按答案类型分类统计
   - 按问题难度分类统计

2. **熵有效性验证**
   - 熵估计 vs 实际步数的相关性
   - 熵估计 vs 人工难度评分的相关性

3. **定理使用分析**
   - 统计每个定理的使用频率
   - 分析定理选择的准确性

4. **错误分析**
   - 分类错误类型（定理选择错误、符号执行错误、答案提取错误）
   - 抽样分析典型错误案例

5. **消融实验**
   - 去除信息增益 → 只用模型预测
   - 去除熵估计 → 固定推理策略
   - 定理库大小的影响

#### 📥 输入
```python
{
    "test_set": "Conic10K/conic10k/test.json",
    "model": trained_model,
    "reasoning_engine": solver
}
```

#### 📤 输出
```python
{
    "overall_accuracy": 0.68,
    "accuracy_by_answer_type": {
        "simple_number": 0.78,
        "complex_number": 0.65,
        "equation": 0.58,
        "coordinate": 0.72
    },
    "entropy_correlation": {
        "pearson": 0.82,
        "spearman": 0.79
    },
    "theorem_usage": {
        "T1_ellipse_abc": 1523,
        "T4_eccentricity": 1205,
        ...
    },
    "error_analysis": {
        "theorem_selection_error": 45,
        "symbolic_execution_error": 28,
        "answer_extraction_error": 12
    },
    "avg_reasoning_steps": 4.8
}
```

#### ✅ 验收标准
- [ ] 完整的评估报告
- [ ] 至少3个消融实验
- [ ] 至少10个详细的案例分析
- [ ] 可视化图表（熵曲线、定理使用分布等）

#### ⏱️ 预计时间
**1.5-2周**

#### 📂 代码文件
- `src/evaluation.py`
- `src/analysis.py`
- `notebooks/results_analysis.ipynb`

---

## 📊 关键指标汇总

| 模块 | 关键指标 | 目标值 |
|------|---------|--------|
| 定理库 | 定理数量 | 15+ (基础) |
| 状态抽象器 | 覆盖率 | > 95% |
| 训练数据 | 样本数 | 50,000+ |
| 神经网络 | 定理选择准确率 | > 70% |
| 神经网络 | 熵估计MAE | < 0.5 |
| 推理引擎 | 端到端准确率 | **60-80%** |
| 推理引擎 | 平均步数 | < 6步 |
| 分析 | 熵与难度相关性 | > 0.75 |

---

## 🎯 里程碑检查点

### ✅ Milestone 1 
- [ ] 15个基础定理实现完成
- [ ] 状态抽象器验证通过
- [ ] 定理应用器核心功能完成

### ✅ Milestone 2 
- [ ] 训练数据构造完成
- [ ] 神经网络训练完成
- [ ] 模型性能达标（定理选择>70%）

### ✅ Milestone 3 
- [ ] 端到端系统集成完成
- [ ] 测试集准确率>60%
- [ ] 完整评估报告

### ✅ Milestone 4 
- [ ] 论文初稿完成
- [ ] 所有实验完成
- [ ] 代码整理和文档完善

---

## 📂 项目目录结构

```
EGR/
├── Conic10K/                    # 数据集
│   └── conic10k/
│       ├── train.json
│       ├── dev.json
│       └── test.json
│
├── src/                         # 源代码
│   ├── state_abstractor.py     # 模块1
│   ├── data_constructor.py     # 模块2
│   ├── theorem_applicator.py   # 模块4
│   ├── reasoning_engine.py     # 模块5
│   ├── evaluation.py           # 模块6
│   ├── models/
│   │   ├── egr_model.py        # 模块3
│   │   └── tokenizer.py
│   └── theorems/
│       ├── theorem_library.py  # 模块0
│       └── basic_theorems.py
│
├── configs/                     # 配置文件
│   ├── model_config.yaml
│   └── training_config.yaml
│
├── tests/                       # 单元测试
│   ├── test_state_abstractor.py
│   ├── test_theorem_applicator.py
│   └── ...
│
├── notebooks/                   # 实验分析
│   ├── data_analysis.ipynb
│   └── results_analysis.ipynb
│
├── outputs/                     # 输出结果
│   ├── models/                  # 训练好的模型
│   ├── results/                 # 评估结果
│   └── logs/                    # 训练日志
│
└── doc/                         # 文档
    ├── phase1_project_workflow.md  # 本文档
    ├── theorem_library_spec.md     # 定理库规范
    └── paper_draft.md              # 论文草稿
```

---

## 🚀 快速开始指南

### 步骤1: 环境配置
```bash
# 安装依赖
pip install torch transformers numpy scipy sympy pandas

# 验证数据集
python scripts/verify_dataset.py
```

### 步骤2: 运行数据分析（模块0的第一步）
```bash
python src/theorems/analyze_reasoning_patterns.py \
    --data Conic10K/conic10k/train.json \
    --output outputs/theorem_statistics.json
```

### 步骤3: 实现第一个定理
```bash
# 参考 src/theorems/basic_theorems.py
# 实现 T5_extract_params 定理
```

### 步骤4: 测试状态抽象器
```bash
python tests/test_state_abstractor.py
```

---

## 📖 相关文档

- [定理库详细规范](theorem_library_spec.md)
- [状态表示设计文档](state_representation.md)
- [模型架构说明](model_architecture.md)
- [实验设置与超参数](experiment_setup.md)

---

## 💡 注意事项

### 关于创新性
1. **最大熵原理的三层应用**是核心理论创新，需要在论文中详细阐述
2. **状态抽象**是工程创新，需要证明有限状态的有效性
3. **熵驱动推理**是方法创新，需要通过消融实验证明其价值

### 关于工程实现
1. 优先实现最常用的15个定理，确保基础功能可用
2. 状态抽象的设计直接影响后续所有模块，需要谨慎设计
3. 神经网络可以先用小模型快速验证，再扩大规模

---

**文档版本**: v1.0  
**最后更新**: 2026-01-08  

