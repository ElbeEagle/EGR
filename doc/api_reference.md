...
| `BoundAction`（`src.theorems.bound_application`） | `model_id, mode, curve, equation_id`；可选 `line_equation_id / relation_id / peer_curve / peer_equation_id`；明确模型、应用方式和所属对象／方程 |
| `enumerate_actions(state, model_id)`（同上） | 枚举结构绑定候选，目前支持 RM3/5/11/12/21；不保证候选已满足所有数学前提 |
...
- `Proposal`：`properties / values / constraints / frames / read_facts / operations / candidates`。模型提出变化，提交由应用器统一完成。
...
非 `applied` 返回不提交状态变化。新增条件／几何框架与属性、赋值一起校验并提交，纳入 delta 和来源；条件矛盾检查目前限可判定的常量／一元条件。已知标量回代仅归约已有属性并记录操作，不引入新定理或要求选择器额外预测 RM78。
...
| RM11 `derive_c_sq` | 已有同一椭圆标准参数与框架 → c²=a²−b² |
...
当前解析只覆盖切片需要的声明、算术、等式、比较条件、已知渐近线和 `Focus(G)=Focus(H)`；查询限已声明标量。参数求解限至多二次的一元实多项式。ID 2 缺少 m>0 时保留 ±5，不默认取正根。两个入口分别要求唯一渐近线／共焦点绑定；尚无多分支搜索、通用超时或 RM21 正向模式的新接口。RM12 尚不支持其他曲线组合或一般参数关系模式。
...
扩展模型时，在原模型类增加绑定模式，复用 `transition_primitives.py` 中的公共计算，通过 `Proposal` 交给应用器；不要为每个模型另建状态或提交逻辑。跨曲线关系已实现上述有限共焦点模式；其他关系、分支状态及统一训练 schema 仍需按规格逐批实现。

快速检查：`python3 -m pytest -q tests/test_bound_transition_slice.py tests/test_bound_shared_focus.py`。

ID 9 回放：`python3 -m src.reasoning.bound_slice --mode shared-focus --problem-id 9`。当前 CLI 输出 `bound-slice-v2`（新增关系／另一对象绑定、框架及推导条件）；旧 ID 2 trace 保留为当批 v1 历史证据。
